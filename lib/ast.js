...
export class ASTSearcher {
    ...
    isLocationAtStartOfSpan(location) {
        return this.checkStartsInRegion(this.doc, location);
    }
    checkStartsInRegion(region, location) {
        var found = false;
        for (var segment of region.segments) {
            if (spanContainsPoint(segment.span, location)) {
                if (segment.span.start[0] === location.row && segment.span.start[1] === location.column)
                    return true;
                else if (segment.type === 'text')
                    return false;
                else
                    return this.checkStartsInRegion(segment.thenbranch, location) || this.checkStartsInRegion(segment.elsebranch, location);
            }
        }
    }
    checkEndsInRegion(region, location) {
        for (var segment of region.segments) {
            if (inclusiveSpanContainsPoint(segment.span, location)) {
                if (segment.span.end[0] === location.row && segment.span.end[1] === location.column)
                    return true; //this means it's at the end of a span
                else if (segment.type === 'text')
                    return false; //if it's not at the end, and this is a text node, finish the search
                else
                    return this.checkEndsInRegion(segment.thenbranch, location) || this.checkEndsInRegion(segment.elsebranch, location); //not at the end of this node, but could be at the start of some subnode
            }
        }
    }
    isLocationAtEndOfSpan(location) {
...
export class EditPreserver extends SyntaxWalker {
    ...
    visitRegion(region) {
        ...
        for (var i = 0; i < region.segments.length; i++) {
            ...
            if (region.segments[i].type === "text" && (i + 1 < region.segments.length) && region.segments[i + 1].type === "text") {
                region.segments[i].content = region.segments[i].content + region.segments[i + 1].content;
    ...
    visitChoice(node) {
        ...
        if (recurseThen)
            changes = this.visitRegion(node.thenbranch) || changes;
        if (recurseElse)
            changes = this.visitRegion(node.elsebranch) || changes;
        return changes;
    }
}
...
function spanContainsPoint(outer, inner) {
    return (((outer.start[0] === inner.row && outer.start[1] <= inner.column) // inclusive at the beginning, exclusive at the end
        ||
