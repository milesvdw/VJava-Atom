...
export class SpanWalker extends SyntaxWalker {
    ...
    visitContent(node) {
        const endPos = this.accumulate(this.currentPos, node.content.slice(0, -1));
        node.span = {
        ...
        for (var segment of region.segments) {
            if (inclusiveSpanContainsPoint(segment.span, location)) {
                if (segment.span.end[0] === location.row && segment.span.end[1] === location.column)
                    ...
                    return this.checkEndsInRegion(segment.thenbranch, location) || this.checkEndsInRegion(segment.elsebranch, location);
            }
        }
    }
