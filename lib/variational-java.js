  ...
  renderDimensionUI(editor) {
    ...
    for(var i = 0; i < vjava.ui.dimensions.length; i ++) {
      (function (i) {
        ...
        if(!vjava.dimensionColors[dimension.dimension]) vjava.dimensionColors[dimension.dimension] = {left: "rgba(102, 102, 153, 0.34)", right: "rgba(41, 193, 47, 0.23)"};
        var leftcolor = vjava.dimensionColors[dimension.dimension].left;
        var rightcolor = vjava.dimensionColors[dimension.dimension].right;

        //add the colors and borders as styles to the document head
        $('head').append(`<style>
          atom-text-editor::shadow .${getLeftCssClass(dimension.dimension)} > span {
            background-color: ${leftcolor} !important;
            display: inline-block !important;
          }
          #${dimension.dimension}-left-text {
          ...
          $('head').append(`<style>
            ...
            #${dimension.dimension}-right-text {
              background-color: ${rightcolor};
            }
            </style>`);
